...
func GetCookies(L *lua.LState) int {
	client := checkClient(L)

	o := reflect.ValueOf(client.Jar)

	entries := reflect.Indirect(o).FieldByName("entries")
	i := GetUnexportedField(entries)
...
func SetCookie(L *lua.LState) int {
	client := checkClient(L)
	url_s := L.CheckString(2)
	key := L.CheckString(3)
	value := L.CheckString(4)

	url,_ := url.Parse(url_s)
